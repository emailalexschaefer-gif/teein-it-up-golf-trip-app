...
// setup time, before Begin Round — matches the DB-level lock (migration
// 037: side_comps rows and rounds.powerplay_hole_number both become
// immutable the moment the round leaves 'upcoming'). hole_number is a
// plain number, not a FK to a holes row — holes don't exist yet at this
// point (they're only generated by begin_round()), so the config has to
// reference a hole by number, matching the existing (dormant) side_comps
// table's own column choice.
export interface WizardSideComp {
  comp_type: 'nearest_pin' | 'longest_drive' | 'pros_approach'
  enabled: boolean
