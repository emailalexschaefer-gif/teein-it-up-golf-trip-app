...
export default function LoginPage() {
  ...
  async function handleMagic(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: {
        // Derive URL at runtime — no env var needed
  ...
  return (
    <>
      ...
      <form onSubmit={mode === 'magic' ? handleMagic : handlePassword} className="space-y-3">
        ...
        {mode === 'password' && (
          <div>
            <label className="block text-sm font-medium text-text mb-1">Password<span className="text-red-500 ml-0.5">*</span></label>
            <input
