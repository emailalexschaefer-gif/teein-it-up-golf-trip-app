    ...
    >
      <div className="flex items-center gap-3">
        <div className="flex-shrink-0">
          {trip.logo_url ? (
            <img src={trip.logo_url} alt={trip.name} className="w-12 h-12 rounded-xl object-cover" />
          ) : (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className={cn('flex-shrink-0 text-xs font-medium px-2 py-0.5 rounded-full', TRIP_STATUS_COLORS[trip.status])}>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {trip.round_count > 0 && (
              <span className="text-xs text-text-subtle">{trip.round_count} round{trip.round_count !== 1 ? 's' : ''}</span>
            )}
            {trip.player_count > 0 && (
              <span className="text-xs text-text-subtle">{trip.player_count} member{trip.player_count !== 1 ? 's' : ''}</span>
            )}
          </div>
        </div>
